...
class HomelabBot:
    ...
    def __init__(self):
        ...
        if user_ids_str:
            try:
                self.allowed_users = [int(uid.strip()) for uid in user_ids_str.split(",")]
            except ValueError:
                logger.error("Invalid TELEGRAM_USER_IDS format. Expected comma-separated integers.")
                raise
    ...
    async def _show_server_details(self, query, server_name: str):
        ...
        if server.get("plug"):
            if server.get("mac"):
                ...
                keyboard.append([InlineKeyboardButton(
                    "⚠️ Cannot power on (no MAC address)",
                    callback_data="noop"
                )])
    ...
    async def _confirm_power_off(self, query, server_name: str):
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Yes, Power Off", callback_data=f"power_off:{server_name}")],
    ...
    async def _power_on_server(self, query, server_name: str):
        ...
        await query.edit_message_text(f"⚡ Powering on *{server_name}*...", parse_mode="Markdown")
    ...
    async def _power_off_server(self, query, server_name: str):
        ...
        await query.edit_message_text(f"🔴 Powering off *{server_name}*...", parse_mode="Markdown")
