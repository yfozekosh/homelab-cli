...
class TestPowerOn:
    ...
    @pytest.mark.asyncio
    async def test_success_first_boot(self, power_service, plug_service, server_service, server):
    ...
    @pytest.mark.asyncio
    async def test_fallback_to_wol(self, power_service, plug_service, server_service, server):
    ...
    @pytest.mark.asyncio
    async def test_failure_turns_off_plug(self, power_service, plug_service, server_service, server):
    ...
    @pytest.mark.asyncio
    async def test_progress_callback_called(self, power_service, plug_service, server_service, server):
        ...
        await power_service.power_on(server, "192.168.1.100", progress_callback=callback)

        assert len(progress_messages) > 0
        assert "Turning on plug..." in progress_messages

    @pytest.mark.asyncio
    async def test_high_power_skips_wol(self, power_service, plug_service, server_service, server):
...
class TestPowerOff:
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown(self, power_service, plug_service, server_service, server):
        ...
        with patch("server.power_service.asyncio.sleep", new_callable=AsyncMock), \
             patch("server.power_service.time.time", side_effect=_fast_forward_time(0, 5)):
    ...
    @pytest.mark.asyncio
    async def test_shutdown_failure_continues(self, power_service, plug_service, server_service, server):
    ...
    @pytest.mark.asyncio
    async def test_timeout_still_turns_off(self, power_service, plug_service, server_service, server):
        """Turns off plug even on timeout"""
        plug_service.get_power.return_value = 50.0  # Never drops

        with patch("server.power_service.asyncio.sleep", new_callable=AsyncMock), \
             patch("server.power_service.time.time", side_effect=_fast_forward_time(0, 20)):
            result = await power_service.power_off(server, "192.168.1.100")
    ...
    @pytest.mark.asyncio
    async def test_progress_callback_receives_power_readings(self, power_service, plug_service, server_service, server):
...
class TestMonitorBoot:
    ...
    async def test_timeout_returns_false(self, power_service, server_service):
        ...
        with patch("server.power_service.asyncio.sleep", new_callable=AsyncMock), \
             patch("server.power_service.time.time", side_effect=_fast_forward_time(0, 10)):
            result = await power_service._monitor_boot(
                {"hostname": "srv1"}, "192.168.1.100", 60, logs.append
            )
    ...
    @pytest.mark.asyncio
    async def test_power_read_failure_does_not_crash(self, power_service, server_service):
        """Continues monitoring even if power read fails"""
        server_service.ping_async.side_effect = [False, False, True]
        power_service.plug_service.get_power = AsyncMock(side_effect=Exception("Plug timeout"))
        logs = []
