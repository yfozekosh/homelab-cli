...
def test_null_values_in_request(api_client):
    response = api_client.post("/plugs", json={"name": None, "ip": "192.168.1.97"})
    assert response.status_code in [400, 422]


def test_wrong_type_in_request(api_client):
    response = api_client.post(
        "/plugs", json={"name": 12345, "ip": "192.168.1.98"}  # Should be string
    )
    assert response.status_code in [200, 400, 422]
...
def test_very_large_request_body(api_client):
    ...
    response = api_client.post(
        "/plugs", json={"name": large_name, "ip": "192.168.1.99"}
    )
